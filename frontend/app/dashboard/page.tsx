...
export default function AdminDashboard() {
  ...
  const setupSocket = () => {
    ...
    socket.on('announcementRemoved', () => {
      // Keep announcements in sync without removing locally
      fetchAnnouncements();
    });

    // Listen for channel updates (join events, member counts)
    socket.off('channelUpdated');
  ...
  const handleCreateGroup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Refresh groups and announcements to get updated data
      await fetchGroups();
      await fetchAnnouncements(); // Announcement is auto-created, refresh to show it
    } catch (err: any) {
  ...
  const handleJoinFromAnnouncement = async (groupId: string, groupName: string, groupCode: string) => {
    try {
      ...
      // Refresh groups and announcements immediately
      await fetchGroups();
      await fetchAnnouncements();
    } catch (err: any) {
      toast.error(err.response?.data?.message || 'Failed to join channel');
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="flex gap-2 items-center">
            <Link href="/groups">
              <Button variant="outline">My Channels</Button>
            </Link>
            <ThemeToggle />
